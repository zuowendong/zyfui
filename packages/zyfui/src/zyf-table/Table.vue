 ...
 * alias为数组格式的情况判断
 */
const arrAlias = (...args: any[]) => {
  const arr = []
  for (let i = 0; i < args.length; i++) arr.push(i)
  return arr
}
 ...
 * }
 */
const selectDataFormat = (item: any, data: any) => {
...
const getFilterData = async() => {
  if (tableFilter.value.length) {
    for (let i = 0; i < tableFilter.value.length; i++) {
      const item: any = tableFilter.value[i]
 ...
 */
const filterModels = ref<any>([])
const filterHandle = (filterList: any) => {
  filterList.forEach((item: any) => {
    ...
    if (!item.opButtons && item.type !== 'date') filterModels.value[item.field] = item[`${item.type}Model`]
  })
}
...
const getTableData = async() => {
  ...
  if (Object.prototype.toString.call(fetch.value) === '[object Function]') {
    ...
    if (res.data) {
      ...
      if (res.pageData) total.value = res.pageData.count || res.pageData.total

      const hasListKey = Object.keys(res.data).findIndex(item => item === 'list')
      if (hasListKey !== -1) tableData.value = res.data.list || []
  ...
  if (tableLayer.value.length && tableData.value.length) tableData.value = [...formatFetchData.value(tableData.value)]
}

const originTableFilter = ref<any>([])
onMounted(() => {
  getFilterData()
...
const refreshHandle = (filterList: any) => {
  originTableFilter.value.forEach((item: any, index: number) => {
    if (item.type === 'date') {
      filterModels.value[item.startDateKey || 'startDate'] = item.dateModel[0] ?? ''
      filterModels.value[item.endDateKey || 'endDate'] = item.dateModel[1] ?? ''
    }
...
// 分页
const pageNumChangeHandle = (val: number) => {
  pageNum.value = val
  getTableData()
}
 ...
 * 列表单项宽度
 */
const getColumnWidth = (style: any) => {
  let widthObj: any = { width: null, minWidth: null }
  if (style) {
    if (style.width && !style.minWidth) widthObj = { width: style.width, minWidth: null }

    if (!style.width && style.minWidth) widthObj = { width: null, minWidth: style.minWidth }
  }
  else {
